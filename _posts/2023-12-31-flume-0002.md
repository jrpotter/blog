...
[Phoenix](/snapshots/flume-0002/phoenix). The goal was to include per-schema
tenancy using the Postgres [Ecto](/snapshots/flume-0002/ecto-postgres) adapter
(schema here referring to database schemas, *not* Ecto schemas). This post will
cover the main components implemented to support this functionality:
...
My initial attempt was to run the [Ecto.Migrator](/snapshots/flume-0002/ecto-migrator)
from within an [ExUnit](/snapshots/flume-0002/exunit) `setup` call.
Unfortunately, the `Ecto.Migrator` is incompatible with the
[sandbox database adapter](/snapshots/flume-0002/ecto-sandbox) typically used
within Phoenix tests (the migrator requires two distinct database connections
whereas the sandbox requires sharing a single connection). In response I tried
forgoing tenancy from tests altogether, running `tenant_migrations` against
`public` alongside `public_migrations`. This felt a bit too cheap for my liking
though.
...
is not available from within an Elixir [release](/snapshots/flume-0002/elixir-release)
(the mechanism I plan to deploy Flume with), an alternative approach needed to
be found. Fortunately I stumbled across this [blog post](/snapshots/flume-0002/tenancy)
which introduced me to the concept of [dynamic repos](/snapshots/flume-0002/dynamic-repo).
...
Most of the authentication code was generated using the [phx.gen.auth](/snapshots/flume-0002/gen-auth)
