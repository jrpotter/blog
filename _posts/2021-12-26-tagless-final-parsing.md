...
In his [introductory text](/snapshots/tagless-final-parsing/kiselyov-interpreters.pdf),
...
[megaparsec](/snapshots/tagless-final-parsing/megaparsec) to interpret programs
in this language, raising errors on malformed or invalidly-typed inputs. We will
evaluate interpreter performance on inputs such as those generated by
...
- is unsafe. Packaged as a library, there are no typing guarantees we leverage
  ...
  valid.
- suffers from the [expression problem](/snapshots/tagless-final-parsing/expression.txt).
...
thunk stack reminiscient of [foldl](/snapshots/tagless-final-parsing/foldl).
This implies we could fix the issue by evaluating our data strictly with e.g.
[deepseq](/snapshots/tagless-final-parsing/deepseq). The problem is
unfortunately deeper than this though. Consider the definition of
[InfixL](/snapshots/tagless-final-parsing/combinators-expr.html#pInfixL)'s
parser:
...
use it. Since we are working with a [left-factored](/snapshots/tagless-final-parsing/left-factoring)
...
the [eq](/snapshots/tagless-final-parsing/eq) package:
...
and so on, where `~` refers to [equality constraints](/snapshots/tagless-final-parsing/equality-constraints).
...
[dynamics](/snapshots/tagless-final-parsing/data-dynamic) library, we'll
build our own for exploration's sake.
...
explicit sharing as noted [here](/snapshots/tagless-final-parsing/kiselyov-embedded.pdf),
but this post is already getting too long. I covered only a portion of the
topics Oleg Kiselyov wrote about, but covered at least the majority of topics
I've so far been exploring in my own personal projects. I will note that the
tagless final approach, while certainly useful, also does add a fair level of
cognitive overhead in my experience. Remembering the details around dynamics
especially is what prompted me to write this post to begin with.

&nbsp;

---

[^1]: Code here and elsewhere found in the [tagless-final-parsing](https://git.jrpotter.com/blog/tagless-final-parsing) repository.
